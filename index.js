...
function renderCard(cardData, themeKey, charName, mesId, fontSizePct = 100, ratioKey = 'landscape', textColorOverride = null, bgColorOverride = null) {
    ...
    // 카드 높이는 항상 비율에서 정해진 고정값 사용 (글씨 크기와 무관)
    const H = _baseH;

    // 사용 가능한 텍스트 영역 높이 (상단 패딩 + 하단 패딩 + 메타 텍스트 영역만 제외)
    const PAD_TOP = 60;
    const availableTextH = H - PAD_TOP - PAD_BOTTOM - 20;

    const measureCanvas = document.createElement('canvas');
    const mctx = measureCanvas.getContext('2d');
    const maxTextWidth = W - PAD_X * 2;

    // 요청한 글씨 크기로 먼저 계산해보고, 카드를 넘치면 자동으로 축소
    ...
    // 텍스트가 카드 높이를 초과하면 글씨를 줄여서 맞춤 (최소 50%까지)
    while (contentHeight > availableTextH && scale > 0.5) {
        scale -= 0.05;
        blocks = computeBlocks(scale);
        contentHeight = totalHeight(blocks);
    }
...
// ── 메시지 버튼 (발췌 팝업 열기) ─────────────────────────
function attachButton(mesEl) { /* 메시지 버튼 비활성화 */ }

function attachAllButtons() { /* 메시지 버튼 비활성화 */ }
