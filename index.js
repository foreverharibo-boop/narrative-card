...
function openPreviewPopup(mesEl) {
    ...
    TEXT_COLORS.forEach(col => {
        ...
        if (col.value === null) {
            sw.style.background = 'linear-gradient(135deg,#d4a017 50%,#1c1a17 50%)';
            sw.style.borderColor = '#fff';
        } else {
            sw.style.background = col.value;
        }
        if (_previewState.textColor === col.value) sw.style.borderColor = '#fff';
        sw.addEventListener('click', () => {
            _previewState.textColor = col.value;
            colorWrap.querySelectorAll('div').forEach(s => { s.style.borderColor = 'transparent'; s.style.transform = ''; });
            sw.style.borderColor = '#fff'; sw.style.transform = 'scale(1.2)';
            doPreview();
        });
    ...
    cpWrap.style.cssText = 'width:22px;height:22px;border-radius:50%;border:2px solid rgba(255,255,255,0.3);overflow:hidden;position:relative;cursor:pointer;flex-shrink:0;';
    cpWrap.style.background = '#888';
    const cpInput = document.createElement('input');
    cpInput.type = 'color'; cpInput.value = '#ffffff';
    cpInput.style.cssText = 'position:absolute;inset:0;opacity:0;width:100%;height:100%;cursor:pointer;';
    cpInput.addEventListener('input', () => {
        _previewState.textColor = cpInput.value;
        cpWrap.style.background = cpInput.value;
        colorWrap.querySelectorAll('div:not(:last-child)').forEach(s => { s.style.borderColor = 'transparent'; });
        doPreview();
    });
...
function renderCard(cardData, themeKey, charName, mesId, fontSizePct = 100, ratioKey = 'landscape', textColorOverride = null) {
    ...
    blocks.forEach((b, i) => {
        ...
        ctx.fillStyle = textColorOverride || (b.isQuote ? theme.text : theme.sub);
