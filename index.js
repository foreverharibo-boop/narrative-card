...
function openPreviewPopup(mesEl) {
    ...
    const overlay = document.createElement('div');
    overlay.className = 'ncard-popup-overlay';
    overlay.style.cssText = 'position:fixed!important;inset:0!important;background:rgba(0,0,0,0.85)!important;z-index:2147483647!important;display:flex!important;align-items:center!important;justify-content:center!important;transform:none!important;overflow-y:auto!important;';
    ...
    prevWrap.style.cssText = 'display:flex;justify-content:center;align-items:center;padding:14px;background:#111;min-height:140px;';
    ...
    function ctrlRow(labelText, contentEl) {
        ...
        lbl.style.cssText = 'font-size:12px;color:rgba(255,255,255,0.6);min-width:68px;flex-shrink:0;';
    ...
    themeSelect.style.cssText = 'flex:1;background:#2a2723;color:#ddd;border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:5px 8px;font-size:12px;';
    THEMES.forEach(t => {
    ...
    fsVal.style.cssText = 'font-size:12px;color:rgba(255,255,255,0.55);min-width:36px;text-align:right;';
    ...
    function selectSwatch(targetSw, colorVal) {
        ...
        targetSw.style.borderColor = '#fff';
    ...
    TEXT_COLORS.forEach(col => {
        if (col.value === 'custom') {
            ...
            cpInput.style.cssText = 'position:absolute;inset:0;opacity:0;width:100%;height:100%;cursor:pointer;';
            cpInput.addEventListener('input', () => {
                _customColorVal = cpInput.value;
                selectSwatch(cpWrap, cpInput.value);
            });
            cpWrap.addEventListener('click', () => cpInput.click());
...
function openResultPopup(dataUrl, savedId) {
    const overlay = document.createElement('div');
    overlay.className = 'ncard-popup-overlay';
    overlay.style.cssText = 'position:fixed!important;inset:0!important;background:rgba(0,0,0,0.82)!important;z-index:2147483647!important;display:flex!important;align-items:center!important;justify-content:center!important;transform:none!important;';
    ...
    imgWrap.style.cssText = 'display:flex;justify-content:center;padding:14px;background:#111;';
...
function openCardViewer(dataUrl) {
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed!important;inset:0!important;background:rgba(0,0,0,0.92)!important;z-index:2147483647!important;display:flex!important;align-items:center!important;justify-content:center!important;transform:none!important;cursor:zoom-out;';

    const img = document.createElement('img');
    img.src = dataUrl;
    img.style.cssText = 'max-width:92vw;max-height:92vh;border-radius:12px;box-shadow:0 8px 40px rgba(0,0,0,0.8);cursor:default;object-fit:contain;';
    img.addEventListener('click', (e) => e.stopPropagation());

    const dlBtn = document.createElement('button');
    dlBtn.textContent = '💾 저장';
    dlBtn.style.cssText = 'position:absolute;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 28px;border:none;border-radius:20px;background:#d4a017;color:#1c1a17;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,0.5);';
...
async function openGallery() {
    ...
    function renderGrid(cardList) {
        ...
        cardList.forEach(c => {
            ...
            item.style.cssText = 'position:relative;border-radius:8px;overflow:hidden;background:rgba(255,255,255,0.04);';
            ...
            btnDel.addEventListener('click', async () => {
                ...
                item.remove();
                const rem = grid.querySelectorAll('div[style]').length;
                if (countSpan) countSpan.textContent = '📜 서술 카드 갤러리 (' + rem + ')';
                if (rem === 0) grid.innerHTML = '<p style="color:#999;font-size:13px;grid-column:1/-1;">아직 생성된 카드가 없습니다.</p>';
            });
