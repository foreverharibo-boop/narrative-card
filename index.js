...
.ncard-modal{background:#ffffff;border-radius:12px;width:min(92vw,900px);max-height:86vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.25);}
...
function openCardViewer(dataUrl) {
    ...
    const img = document.createElement('img');
    img.src = dataUrl;
    img.style.cssText = 'max-width:90vw;max-height:80vh;max-height:80dvh;border-radius:12px;box-shadow:0 8px 40px rgba(0,0,0,0.8);cursor:default;object-fit:contain;display:block;';
    ...
    function renderGrid(cardList) {
        ...
        cardList.forEach(c => {
            ...
            btnDel.addEventListener('click', async () => {
                ...
                if (rem === 0) grid.innerHTML = '<p style="color:#666;font-size:13px;grid-column:1/-1;text-align:center;padding:40px 0;">아직 생성된 카드가 없습니다.</p>';
            });
